...
CAPTION RULES:
- 100-150 words. Personal voice. First person.
- End with one open question OR a DM/comment CTA — never both.
- No banned words: "In conclusion", "Dive into", "Leverage", "Game-changer", "Synergy"

DM KEYWORD:
- One word (e.g. TYPES, PANDAS, STIGMA, POEM) that viewers comment to receive the blog/resource link.
...
def process_slug(week: str, slug: str, project_key: str | None = None) -> bool:
    ...
    prompt = build_prompt(account, niche, blog, srt, project_key)
...
def main():
    ...
    ok = sum(process_slug(args.week, s, args.project) for s in slugs)
