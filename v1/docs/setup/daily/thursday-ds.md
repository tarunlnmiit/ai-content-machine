---
title: "Thursday — DS Track (~1.5 hrs)"
type: doc
...
> **DS 2-blog cadence:** Thursday is also when the weekly DS **NEWS/OPINION** blog gets generated (the second DS blog). Monday = TUTORIAL; Thursday = NEWS. Generate it before starting the render queue.

### Generate DS NEWS/OPINION blog (before rendering)
```bash
python3 scripts/produce_blog.py \
  ...
  --humanize
```
The script will suggest 5 editorial/trend topics → you pick → creator input → 5 title options → generate.
Output: `content/blogs/{week}/YYYY-MM-DD_data_science_tech_{slug}.md`

Then repurpose (one command — runs Phase 1 text derivatives + Phase 2: worksheet outline, blog CTA, slide deck, IG carousel, IG reel brief, YT filming script, LinkedIn 2nd comment):
```bash
